...
export class Listener<T extends Record<string, any> = Record<string, any>> {
  ...
  protected registerListener<K extends keyof T>(event: K, callback: (payload: T[K]) => void | Promise<void>): void {
...
export class Emitter<T extends Record<string, any> = Record<string, any>> {
  ...
  on<K extends keyof T>(event: K, listener: (payload: T[K]) => void | Promise<void>): CleanupCallback {
